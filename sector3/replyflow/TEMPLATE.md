...
| **AI follow-up chat** | `Sector3ToolChatModal` + `/api/sector3/[slug]/chat` | Per-tool chat config in `chat-content.ts` with custom welcome, button label, and system prompt context. Wire chat only where the tool benefits (GrantBot, Signal Desk, GapScan, BridgeAI). ReplyFlow typically skips chat. **New Chat** starts a fresh conversation. Users can delete or archive chats (archived auto-delete in 7 days unless revived). Messages persist in `sector3_conversations` + `sector3_chat_messages`. |
| **Prompt clarification** | `Sector3ClarificationPanel` + `/api/sector3/[slug]/clarify` | If the user's prompt is too vague, AI asks follow-up questions with selectable options (multi-select) and custom typed answers before generation. Detailed prompts skip straight to generate. |
| **User context learning** | `sector3_user_context` | Each generate/chat updates per-user context JSON so tools learn preferences over time. |
