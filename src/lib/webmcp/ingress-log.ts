...
export async function logIngress(tool: ManifestTool, params: Record<string, unknown>, ctx: ToolContext, result: ToolResult) {
  ...
  try {
    ...
    await ctx.supabase.from("axon_agent_messages").insert({
      ...
      meta: { tool_name: tool.name, client_key: clientKeyHash(ctx.req.headers), parameters: params, engine: ctx.engine, signature: ctx.signature, result_status: result.status, timestamp: new Date().toISOString() },
    });
  } catch (err) {
