...
import { pickAxonModelForComplexity } from './prefer-axon-local-models.mjs';
...
export async function runLocalModelDaily(opts = {}) {
  ...
  // Lead scoring is a short, structured task (score 0-100 + one-line why) -- route it to
  // the fast/cheap axon-llama tag instead of the heavier axon-ornith reasoning model.
  // (2026-08-26, JB direct order "wire that in" -- ornith/llama complexity routing.)
  const resolvedModel = pickAxonModelForComplexity(probe.models || [], 'simple', model);
