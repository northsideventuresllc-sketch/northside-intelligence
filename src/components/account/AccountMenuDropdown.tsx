...
type AccountMenuVariant = "portal" | "replyflow" | "grantbot";
...
export function AccountMenuDropdown({
  ...
  const triggerClass = isReplyflow
    ...
    : isGrantbot
      ? "rounded-lg border border-gb-emerald/30 bg-gb-emerald/10 px-3 py-1.5 text-sm font-medium text-gb-emerald transition hover:border-gb-teal/40 hover:bg-gb-emerald/20"
  ...
  const panelClass = isReplyflow
    ...
    : isGrantbot
      ? "absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-xl border border-white/10 bg-gb-bg/95 shadow-xl backdrop-blur-xl"
  ...
  const linkClass = isReplyflow
    ...
    : isGrantbot
      ? "block px-4 py-2.5 text-sm text-gb-muted transition hover:bg-white/5 hover:text-gb-emerald"
            ...
            <SignOutButton
              className={
                isReplyflow
                  ...
                  : isGrantbot
                    ? "w-full rounded-lg border border-white/10 px-4 py-2 text-sm text-gb-muted transition hover:border-gb-emerald/30 hover:text-gb-emerald disabled:opacity-50"
                    : undefined
              }
            />
          </div>
