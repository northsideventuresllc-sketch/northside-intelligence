...
import { AssistantServiceCard } from "@/components/services/AssistantServiceCard";
...
const QUICK_PROMPTS = [
  "Personal automation under $500",
  "Automate my research workflow",
  "Business AI strategy",
  "Connect my team's tools",
] as const;

const WELCOME_MESSAGE =
  "Hi — I'm your Intelligence Services assistant. Tell me what you want to accomplish (personal or business), your budget, and I'll recommend the best service from our catalog.";

interface ServiceAssistantWidgetProps {
  onServiceSelect?: (slug: string) => void;
}

export function ServiceAssistantWidget({ onServiceSelect }: ServiceAssistantWidgetProps) {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      const res = await fetch("/api/services/assistant", {
        method: "POST",
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: data.message ?? "Here are some services that may fit.",
    ...
    <div className="fixed bottom-4 left-4 z-50 flex flex-col items-start gap-3">
      {open && (
        <div
          ...
          role="dialog"
          aria-label="Intelligence Services assistant"
        >
          <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
            <div>
              <p className="text-sm font-semibold text-white">Services Assistant</p>
      ...
      <button
        ...
        aria-expanded={open}
        aria-label={open ? "Close services assistant" : "Open services assistant"}
      >
